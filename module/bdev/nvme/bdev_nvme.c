...
bdev_nvme_create_qpair(struct nvme_qpair *nvme_qpair)
{
	...
	SPDK_NOTICELOG("[DEBUG] calling spdk_nvme_ctrlr_alloc_io_qpair\n");
	...
	SPDK_NOTICELOG("[DEBUG] calling spdk_nvme_poll_group_add\n");
	...
	SPDK_NOTICELOG("[DEBUG][0] trying last call spdk_nvme_ctrlr_connect_io_qpair\n");
	rc = spdk_nvme_ctrlr_connect_io_qpair(nvme_ctrlr->ctrlr, qpair);
	if (rc != 0) {
		NVME_CTRLR_ERRLOG(nvme_ctrlr, "Unable to connect I/O qpair.\n");
		goto err;
	}
	SPDK_NOTICELOG("[DEBUG][0] completed last call spdk_nvme_ctrlr_connect_io_qpair\n");
			     ...
			     void *ctx)
{
	...
	if (nvme_qpair->qpair == NULL) {
		SPDK_NOTICELOG("[DEBUG] calling bdev_nvme_create_qpair\n");
...
bdev_nvme_create_ctrlr_channel_cb(void *io_device, void *ctx_buf)
{
	...
	SPDK_NOTICELOG("[DEBUG] calling nvme_qpair_create\n");
		       ...
		       struct nvme_async_probe_ctx *ctx)
{
	...
	SPDK_NOTICELOG("[DEBUG] calling spdk_io_device_register for %s\n", nvme_ctrlr->nbdev_ctrlr->name);
		  ...
		  struct nvme_async_probe_ctx *ctx)
{
	...
	if (cdata->cmic.ana_reporting) {
		...
		SPDK_NOTICELOG("[DEBUG] calling nvme_ctrlr_create_done\n");
		nvme_ctrlr_create_done(nvme_ctrlr, ctx);
		return 0;
	}
	  ...
	  struct spdk_nvme_ctrlr *ctrlr, const struct spdk_nvme_ctrlr_opts *drv_opts)
{
	...
	SPDK_NOTICELOG("[DEBUG] calling nvme_ctrlr_create\n");
		      ...
		      struct spdk_bdev_nvme_ctrlr_opts *bdev_opts)
{
	...
	if (nvme_bdev_ctrlr_get_by_name(base_name) == NULL || ctx->bdev_opts.multipath) {
		SPDK_NOTICELOG("[DEBUG] calling connect_attach_cb\n");
